...
RegisterNetEvent('CXRP:TakenTurf')
AddEventHandler('CXRP:TakenTurf', function(isnTurf)
...
RegisterNetEvent('CXRP:OutOfZone')
AddEventHandler('CXRP:OutOfZone', function(isnTurf)
	inTurf = false
...
RegisterNetEvent('CXRP:PlayerDied')
AddEventHandler('CXRP:PlayerDied', function(isnTurf)
	inTurf = false
...
RegisterNetEvent('CXRP:TurfComplete')
AddEventHandler('CXRP:TurfComplete', function(reward, name)
	inTurf = false
...
Citizen.CreateThread(function()
	while true do
		...
		if inTurf then		
			...
			TriggerServerEvent('CXRP:PlayerDied', turf)

			elseif (Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) > turfs[turf].radius) then
				TriggerServerEvent('CXRP:TooFar', turf)
			end
		end
