...
RegisterCommand("purge", function(source,args,rawCommand)
    purge = not purge
    print(purge)
    TriggerServerEvent('CXRP:activatePurgeSV', purge)
end)

RegisterCommand("testgun", function(source,args,rawCommand)
    if purge then
        TriggerServerEvent('CXRP:purgeRespawn')
    end
end)
...
RegisterNetEvent("CXRP:activatePurge")
AddEventHandler("CXRP:activatePurge", function()
