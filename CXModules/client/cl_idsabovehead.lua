...
RegisterCommand("farids", function()
    if faridsenabled then
        TriggerServerEvent("CXRP:IDsAboveHead", true)
    else
        TriggerServerEvent("CXRP:IDsAboveHead", false)
    end
end)
...
RegisterCommand("hidechat", function()
    TriggerEvent('CXRP:toggleChat', true)
    notify('~g~Chat Hidden')
end)

RegisterCommand("showchat", function()
    TriggerEvent('CXRP:toggleChat', false)
    notify('~g~Chat Shown')
end)

RegisterNetEvent("CXRP:ChangeIDs")
AddEventHandler("CXRP:ChangeIDs", function(status)
    if status then
        disPlayerNames = 10
