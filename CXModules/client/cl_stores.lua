...
RMenu.Add("CXRP:Shops", "main", RageUI.CreateMenu("Shop", "~b~Shop", GetRageUIMenuWidth(),GetRageUIMenuHeight()))
RMenu.Add("CXRP:Shops", "sub", RageUI.CreateSubMenu(RMenu:Get("CXRP:Shops", "main"), "Shop", "~b~Confirm Purchase", GetRageUIMenuWidth(),GetRageUIMenuHeight()))

RageUI.CreateWhile(1.0, RMenu:Get("CXRP:Shops", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("CXRP:Shops", "main"), true, false, true, function()
        for k, v in pairs(cfg.items) do
            if v.type == 'store' then
                ...
                end, RMenu:Get("CXRP:Shops", "sub", true))
            end
        end
    ...
    RageUI.IsVisible(RMenu:Get("CXRP:Shops", "sub"), true, false, true, function()
        ...
        RageUI.ButtonWithStyle("~g~Confirm Purchase",nil,  {RightLabel = "£"..getMoneyStringFormatted(currentItemPrice * amount)}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("CXRP:BuyShopItem", currentItemID, amount)
            end
        end, RMenu:Get("CXRP:Shops", "main", true))
        ...
        for k, v in pairs(cfg.shops) do
            ...
            if isInArea(v, 1.0) and inMenu == false then
                ...
                if IsControlJustPressed(0, 51) then 
                    inMenu = true
                    currentShop = k
                    RageUI.Visible(RMenu:Get("CXRP:Shops", "main"), true)
                end
            end
