...
RegisterCommand("carry",function(source, args)
    if not carry.InProgress then
        ...
        if closestPlayer then
            ...
            if targetSrc ~= -1 then
                TriggerServerEvent("CXRP:CarryRequest", targetSrc, OMioDioMode)
            else
...
RegisterNetEvent("CXRP:StartCarry")
AddEventHandler("CXRP:StartCarry", function(targetSrc)
...
RegisterNetEvent("CXRP:CarryTargetAsk")
AddEventHandler("CXRP:CarryTargetAsk", function(senderSrc)
    carryrequest = true
    Citizen.CreateThread(function()
        while carryrequest do
            if IsControlJustPressed(1, 83) then
                TriggerServerEvent("CXRP:CarryAccepted",senderSrc)
                carryrequest = false
            elseif IsControlJustPressed(1, 84) then
                TriggerServerEvent("CXRP:CarryDeclined",senderSrc)
                carryrequest = false
            end
            Wait(0)
        end
