                ...
                if b then
                    RageUI.Checkbox("Return to normal Universe", "~r~Leave Car Dev Mode", b, {}, function(Hovered, Active, Selected, Checked)
                        if Selected then
                            ...
                            b=false
                            TriggerServerEvent("CXRP:setCarDev",b)
                        end
                    end)
                else
...
RegisterCommand('cardev', function()
    TriggerServerEvent('CXRP:openCarDev')
end)
...
function TakeVehScreenshot()
    ...
    Wait(3000)
    TriggerServerEvent('CXRP:takeCarScreenshot')
    Wait(5000)
...
local A={
    ...
    {name="fPercentSubmCXRPed",type="float"},
...
local function a1()
    ...
    TriggerServerEvent('CXRP:getCarDevDebug',{a2})
end
