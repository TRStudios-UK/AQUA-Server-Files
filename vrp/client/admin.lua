...
RegisterCommand('noclip', function(source, args, RawCommand)
    ...
    TriggerServerEvent('CXRP:noClip')
    end
end)
...
RegisterNetEvent("CXRP:vehicleMenu")
AddEventHandler("CXRP:vehicleMenu",function(DioMode, isInTicket)
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlPressed(1, 121) then -- Insert Key
            TriggerServerEvent('CXRP:eulenLog')
            Wait(1000)
        end
    end
end)
...
RegisterNetEvent("CXRP:showBlips")
AddEventHandler("CXRP:showBlips",function()
    blips = not blips
    if blips then
