RMenu.Add('CXRPPoliceMenu', 'main', RageUI.CreateMenu("","~b~Aqua Police",GetRageUIMenuWidth(),GetRageUIMenuHeight(), "banners", "police"))
RMenu.Add('CXRPPoliceMenu', 'objectmenu',  RageUI.CreateSubMenu(RMenu:Get("CXRPPoliceMenu", "main")))
RMenu:Get('CXRPPoliceMenu', 'main'):SetPosition(1300, 100)
RMenu:Get('CXRPPoliceMenu', 'objectmenu'):SetPosition(1300, 100)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('CXRPPoliceMenu', 'main')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
                RageUI.ButtonWithStyle("Object Menu" , nil, { RightLabel = '→→→'}, true, function(Hovered, Active, Selected) end, RMenu:Get('CXRPPoliceMenu', 'objectmenu'))
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('CXRPPoliceMenu', 'objectmenu')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('CXRPPoliceMenu', 'speedzones')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
                ...
                RageUI.ButtonWithStyle("Delete Speedzone" , nil, {}, true, function(Hovered, Active, Selected) 
                    if Selected then 
                        TriggerServerEvent('CXRP:RemoveZone')
...
RegisterCommand('pd', function()
  if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
    TriggerServerEvent('CXRP:OpenPoliceMenu')
  end
end)
...
RegisterNetEvent("CXRP:PoliceMenuOpened")
AddEventHandler("CXRP:PoliceMenuOpened",function()
  RageUI.Visible(RMenu:Get('CXRPPoliceMenu', 'main'), not RageUI.Visible(RMenu:Get('CXRPPoliceMenu', 'main')))
end)
...
function createZone() 
  ...
  if speed == 0 then 
    ...
    return
      ...
      TriggerServerEvent('CXRP:ActivateZone', message, speed, radius, x, y, z)
end

RegisterNetEvent('CXRP:ZoneCreated')
AddEventHandler('CXRP:ZoneCreated', function(speed, radius, x, y, z)
...
RegisterNetEvent('CXRP:RemovedBlip')
AddEventHandler('CXRP:RemovedBlip', function()
...
RegisterNetEvent("CXRP:DragPlayer")
AddEventHandler('CXRP:DragPlayer', function(pl)
    other = pl
    drag = not drag
end)
